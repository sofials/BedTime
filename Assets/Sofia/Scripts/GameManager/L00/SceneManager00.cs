...
public class SceneManager00 : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI levelTitleText; // Campo per il TextMeshPro
    ...
    private void SetupUI()
    {
        ...
        // CAMBIATO: Avvia subito l'animazione del titolo se abilitata
        if (levelTitleUI != null && showLevelTitle)
        {
            DebugLog("[SceneManager00] Avvio immediato animazione titolo livello");
    ...
    private void OnGameManagerSceneReady(string sceneName)
    {
        ...
        {
            DebugLog("[SceneManager] GameManager pronto - attivazione PowerUp UI");

            // CAMBIATO: Attiva solo la PowerUp UI, il titolo è già partito
            EnablePowerUpUI();

            DebugLog("[SceneManager] PowerUp UI attivata, scena pronta");
        }
    }
    /// <summary>
    ...
    private IEnumerator ShowLevelTitleCoroutine()
    {
        ...
        DebugLog("[SceneManager00] Inizio animazione titolo livello");
        ...
        if (titleText == null)
        {
            DebugLog("[SceneManager00] ⚠️ TextMeshProUGUI non trovato. Assegna 'levelTitleText' dall'inspector o verifica 'levelTitleUI'");
        ...
        DebugLog($"[SceneManager00] TextMeshProUGUI trovato: {titleText.name}");
        ...
        // === FADE IN ===
        DebugLog("[SceneManager00] Level Title - Fade In");
        targetColor.a = 0f;
        ...
        titleText.color = targetColor;
        DebugLog("[SceneManager00] Level Title - Completamente visibile");

        // === DISPLAY ===
        yield return new WaitForSeconds(displayDuration);

        // === FADE OUT ===
        DebugLog("[SceneManager00] Level Title - Fade Out");
        elapsed = 0f;
        ...
        isTitleAnimationPlaying = false;
        DebugLog("[SceneManager00] Level Title - Animazione completata");
    }
...
{
    Debug.Log($"Level Title UI: {(levelTitleUI != null ? levelTitleUI.name : "NULL")}");
    Debug.Log($"Level Title Text: {(levelTitleText != null ? levelTitleText.name : "NULL")}");
    Debug.Log($"Show Level Title: {showLevelTitle}");
}
}